...
# old whole brain mask
#mask = index_img(all_cues_img,1)
#mask = new_img_like(mask, (mask.get_fdata()!=0))

# now thalamus mask, only do decoding on thalamus for now
mask = nib.load("/Shared/lss_kahwang_hpc/ROIs/Morel_2.5_mask.nii.gz")
...
if permutation:
    ...
    try:
        ...
        context_score_img = new_img_like(mean_fmri, searchlight_context.scores_)
        #context_score_img.to_filename(func_path + "%s_context_svc_score.nii.gz" %subject)
        thalamus_context_decoding = NiftiMasker(mask).fit_transform(context_score_img)
        np.save("/Shared/lss_kahwang_hpc/data/ThalHi/decoding/permutation/%s_context_decoding.npy" %subject, thalamus_context_decoding)


        #now the permutation
        perumted_thalamus_context_decoding = np.zeros((500, 2473))
        for i in range(500):
            np.random.shuffle(context_Y)
            searchlight_context = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=24, verbose=0, cv=LeaveOneGroupOut())
            searchlight_context.fit(all_cues_img, context_Y, runs)
            context_score_img = new_img_like(mean_fmri, searchlight_context.scores_)
            #context_score_img.to_filename(func_path + "%s_context_svc_score.nii.gz" %subject)
            perumted_thalamus_context_decoding[i,:] = NiftiMasker(mask).fit_transform(context_score_img)[0]
        
        np.save("/Shared/lss_kahwang_hpc/data/ThalHi/decoding/permutation/%s_context_decoding_permutation.npy" %subject, perumted_thalamus_context_decoding)        
